...
public function NoPay(Request $request){
    if($request->inv_type == 'NoPay'){
        if($request->NoPay == 1){
        $NoPay = $request->NoPay;
        $data = array('NoPay' => $NoPay);
    }
    else{
        $NoPay = $request->NoPay;
        $data = array('NoPay' => $NoPay);
    ...
    if($request->inv_type == 'NoNumber'){
    if($request->NoNumber == 1){
        $NoNumber = $request->NoNumber;
        $data = array('NoNumber' => $NoNumber);
    }
    else{
        $NoNumber = $request->NoNumber;
        $data = array('NoNumber' => $NoNumber);
